  ...
  const filteredDeals = useMemo(() => {
    ...
    switch (currentView) {
      ...
      case 'my-deals':
        ...
        break;
      case 'high-value':
        filtered = filtered.filter(deal => deal.value >= 5000000); // ₹50L+
        break;
    ...
    if (filters.valueRange && filters.valueRange !== 'all-values') {
      switch (filters.valueRange) {
        case '0-500000':
          filtered = filtered.filter(deal => (deal.value || 0) >= 0 && (deal.value || 0) <= 500000);
          break;
        case '500000-2500000':
          filtered = filtered.filter(deal => (deal.value || 0) > 500000 && (deal.value || 0) <= 2500000);
          break;
        case '2500000-5000000':
          filtered = filtered.filter(deal => (deal.value || 0) > 2500000 && (deal.value || 0) <= 5000000);
          break;
        case '5000000+':
          filtered = filtered.filter(deal => (deal.value || 0) > 5000000);
          break;
      }
    }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Total: {filteredDeals.length} deals • Value: {formatCurrency(totalValue)} • Weighted: {formatCurrency(weightedValue)}
          </p>
        </div>
